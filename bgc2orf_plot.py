...
def plot_orf2core(d):
    x = [x*1 for x in range(1, len(d)+1)]
    fig, ax1 = plt.subplots()
    plt.rcParams["font.family"] = "Times New Roman"
    
    train_loss, = ax1.plot(x, d[:,1], label = 'train_loss', color='red')
    valid_loss, = ax1.plot(x, d[:,2], label = 'validation_loss', color='blue')
    ...
    ax1.set_title("bgc2orf", fontname="Times New Roman")
    ...
    ax2 = ax1.twinx()
    train_accu, = ax2.plot(x, d[:,3], label = 'train_accuracy', color='green')
    valid_accu, = ax2.plot(x, d[:,4], label = 'validation_accuracy', color='orange')
...
def main():
    infile = "../genome2orf/bgc2orf/models/1026/model_1026.csv"
